...
app.config['SQLALCHEMY_DATABASE_URI']='mysql://root:@localhost/hmsnew'
db=SQLAlchemy(app)
...
@app.route('/meetourdoctors')
def meet_our_doctors():
    # If the user is authenticated and a patient, render the doctor page
...
def bookings():
    ...
    if current_user.usertype == "Doctor":
        ...
        # Filter patients by the doctor's department
        query = Patients.query.filter_by(dept=department).all()
        
        return render_template('booking.html', query=query)
    else:
        # If user is not a doctor, show all their appointments
